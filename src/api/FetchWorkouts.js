...
const getAllWorkouts = async () => {
	try {
		...
		const response = await axios.get('https://workout-backend-c6o4.onrender.com/workouts', {
...
const addWorkout = async (type, date, duration) => {
	try {
		const token = localStorage.getItem('token');
		await axios.post(
			'https://workout-backend-c6o4.onrender.com/workouts/saveWorkout',
			{ type, date, duration },
			{ headers: { Authorization: `Bearer ${token}` } }
		);
	} catch (error) {
...
const editWorkout = async (workoutId, type, date, duration, setWorkout) => {
	try {
		const token = localStorage.getItem('token');
		await axios.post(
			'https://workout-backend-c6o4.onrender.com/workouts/editWorkout',
...
const deleteWorkout = async (workoutId) => {
	try {
		const token = localStorage.getItem('token');
		await axios.delete('https://workout-backend-c6o4.onrender.com/workouts/deleteWorkout', {
			headers: { Authorization: `Bearer ${token}` },
			data: { _id: workoutId },
		});
	} catch (error) {
